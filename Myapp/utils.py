...
def generate(name, job_description, participant_info, level, role, transcript_chat, transcript):
    prompt = (
        f"Act as a real interviewer. this is the job description: {job_description}. "
        f"here is the info from his/her resume {participant_info}. name : {name} "
        f"he is applying for {role} as a {level}. you are having a chat. You will act as an interviewer. "
        f"ask him questions based on his previous chat:{transcript_chat} and resume. "
        f"he just told {transcript}, just ask him a single question noting else should be written."
        f"don't ask any questions that are asked previously, if anyone responses to you in a bad or unprofessional manner "
        f"just tell them to behave properly. Do not tell him that you are the interviewer, instead just pretend as an interviewer."
    )
